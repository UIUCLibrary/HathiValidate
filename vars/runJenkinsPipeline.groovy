                        ...
                        steps{
                            catchError(buildResult: 'UNSTABLE', message: 'Building documentation produced an error or a warning', stageResult: 'UNSTABLE') {
                                sh(script: '''python3 -m venv venv
                                              ...
                                              . ./venv/bin/activate
                                              mkdir -p logs
                                              uvx --from sphinx --with-editable . --with-requirements requirements-dev.txt sphinx-build -b html docs/source build/docs/html -d build/docs/doctrees -w logs/build_sphinx.log -W --keep-going
                                           '''
                                )
                            }
                        }
                            ...
                            stage('Windows'){
                                ...
                                steps{
                                    script{
                                        def envs = []
                                        node('docker && windows'){
                                            docker.image('python').inside('--mount source=python-tmp-hathivalidate,target=C:\\Users\\ContainerUser\\Documents'){
                                                try{
                                                    checkout scm
                                ...
                                stage('Test Package in container') {
                                    ...
                                    agent {
                                        docker {
                                            ...
                                            args "--mount source=python-tmp-hathivalidate,target=${['windows'].contains(OS) ? 'C:\\Users\\ContainerUser\\Documents': '/tmp'}"
                                        }
                                    }
                                    environment{

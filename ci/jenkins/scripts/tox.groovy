                                ...
                                try{
                                    ...
                                    if(isUnix()){
                                        sh(
                                            ...
                                            script: "tox -v --parallel--safe-build --result-json=${TOX_RESULT_FILE_NAME} --workdir=/tmp -e ${tox_env}"
                                        )
                                    } else {
                                        bat(
                                            label: "Running Tox with ${tox_env} environment",
                                            script: "tox -v --parallel--safe-build --result-json=${TOX_RESULT_FILE_NAME} --workdir=%TEMP% -e ${tox_env}"
                                        )
                                    }
                                } catch (e){
                                    def text
